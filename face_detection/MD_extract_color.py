        ...
        if body == 'face_skin':
            ...
            for region in exclude_regions:
                exclude_points = np.array([
                    (int(face_landmarks[idx[0]].x * rgb_image.shape[1]), int(face_landmarks[idx[0]].y * rgb_image.shape[0]))
...
# 얼굴 주요 색상 분석 함수
def analyze_face_colors(img_path):
    rgb_image = load_and_prepare_image(img_path)
...
# 이미지 파일 경로를 입력하여 실행
analyze_face_colors('data/test.png')
