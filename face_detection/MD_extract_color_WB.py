...
def load_and_prepare_image_with_manual_white_balance(img_path):
    ...
    avg_gray = (avg_r + avg_g + avg_b) / 3   # 128로 할지 고민...(내일 여러 연예인 사진에 대해서 알고리즘 두 가지로 테스트해보기), 오늘은 연예인 누구로 할건지 정하기(기준)
...
# 이미지 파일 경로를 입력하여 실행
analyze_face_colors_with_white_balance('data/test.png')
